...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction(o => o match
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction ( o => o match 
      case Json.Arr(items) => 
        val possiblyDecoded: List[Option[A]] = items.map(decoder.decode)
        def flipMonads[A](x: List[Option[A]]): Option[List[A]] = x match
          case Some(a) :: rest => flipMonads(rest) match
            case Some(b) => Some(a :: b)
            case None => None
          case Nil => Some(Nil)
          case None :: rest => None
        flipMonads(possiblyDecoded)
      case _ => None
   )
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform(p => Contacts(p))
...
object Main:
  ...
  def main(args: Array[String]): Unit =
    ...
    val contacts = parseJson("""{"people": [{"name":"Alice","age": 42},{"name": "Bob", "age": 21}] }""")
