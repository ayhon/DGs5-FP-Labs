...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for (neigh, move) <- b.legalNeighbors yield (neigh, move :: history)).to(LazyList)
    
    // (b, history) #:: b.legalNeighbors.foldLeft(LazyList.empty)(
    //   (lazylist: LazyList[(Block, List[Move])], neigh_move: (Block, Move)) => neigh_move match
    //     case (neighbor, move) => (neighbor, move :: history) #:: lazylist
    // )
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors match
                        case (neigh, history) #:: rest => if ! explored.contains(neigh) 
                          then (neigh, history) #:: newNeighborsOnly(rest, explored.incl(neigh))
                          else newNeighborsOnly(rest, explored)
                        case _ => neighbors
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case (block, history) #:: rest => 
      ...
      val newExcluded = explored.union( newStates.unzip._1.toSet )
      newStates ++ from(rest ++ newStates, newExcluded)
