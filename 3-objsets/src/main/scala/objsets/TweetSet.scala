...
abstract class TweetSet extends TweetSetInterface:
   ...
   * 
   * Answer: It can be implemented in terms of the other methods
   */
   ...
   * and be implemented in the subclasses?
   */
  def union(that: TweetSet): TweetSet = filterAcc(!that.contains(_), that)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = 
    ...
    if left.isEmpty && right.isEmpty then
      ...
      max(elem,max(right.mostRetweeted,left.mostRetweeted))

  def descendingByRetweet: TweetList = 
    Cons(this.mostRetweeted, remove(this.mostRetweeted).descendingByRetweet)
...
object GoogleVsApple:
  ...
  def tweetsByTerms(tweets: TweetSet, terms: List[String]): TweetSet = 
    def hasTerms(tweet: Tweet): Boolean =
      terms.exists(
        (word: String) => tweet.text.contains(word)
      )
    tweets.filter(hasTerms)

  lazy val googleTweets: TweetSet = tweetsByTerms(TweetReader.allTweets, google)
  lazy val appleTweets: TweetSet = tweetsByTerms(TweetReader.allTweets, apple)
